...
\begin{enumerate}
    ...
    \item \textbf{Phase~2 keeps two explicit blocklists.} The set \texttt{LEAKY\_COLS\_A} contains 15 columns that would leak in the pre-departure setting; \texttt{LEAKY\_COLS\_B} contains 11 (it is \texttt{LEAKY\_COLS\_A} minus the three inflight whitelist features, plus \texttt{headway\_to\_train\_ahead}). Before the scaler is fit, the active feature list is asserted to be disjoint from the relevant blocklist.
...
\Cref{tab:splits} summarises the temporal split. Train, validation and test are non-overlapping date windows, and \texttt{splits.assert\_no\_service\_overlap} enforces at runtime that no service ID appears in two partitions. The disruption rate drops from \SI{10.34}{\percent} on the four-month training window to \SI{7.85}{\percent}--\SI{7.88}{\percent} on the validation and test months, reflecting a milder spring weather regime.

\begin{table}[htbp]
    \centering
    \caption{Temporal split sizes after Phase~2. ``Disrupted rate'' is the fraction of stops whose arrival is more than five minutes late or that are cancelled. The drop from \SI{10.34}{\percent} (winter training) to \SI{7.88}{\percent} (June test) reflects normal seasonal disruption -- it is not a per-class label drift.}
    \label{tab:splits}
    \small
    \begin{tabular}{l
        ...
        \midrule
        Train      & 11915114 & 10.34 & 2024-01-01 -- 2024-04-30 \\
        Validation &  2368484 &  7.85 & 2024-05-01 -- 2024-05-31 \\
        Test       &  2303236 &  7.88 & 2024-06-01 -- 2024-06-30 \\
        \bottomrule
    \end{tabular}
\end{table}
