...
\paragraph{Artefact map.} After a full run the canonical consumables are: \texttt{Data/stops/stops\_\{train,val,test\}.parquet} (47 columns, scaled), \texttt{feature\_names\_\{A,B\}.json} (the contract), \texttt{scaler\_\{mean,scale\}\_\{A,B\}.npy} (inverse transform), \texttt{edge\_index.npy} + \texttt{edge\_attr.npy} (PyG-compatible station graph), \texttt{stop\_level/results/benchmark\_stops.json} (every metric table), \texttt{stop\_level/results/xai\_report\_stops.json} (SHAP global + 4 local explanations + lag-ablation + KG-bridge Cypher), and 22 figures under \texttt{stop\_level/figures/}.
